...
<template>
    <div>
        <Menu :model="items" class="w-full h-full md:w-60">
            <template #start>
                <span class="inline-flex items-center gap-1 px-2 py-2">
                    ...
                    <span class="text-xl font-semibold">Test<span class="text-primary">Master</span></span>
                    <Button
                        @click="toggleTheme"
                        class="p-button-text p-button-plain"
                        :icon="themeIcon"
                        v-tooltip="isDarkMode ? 'Переключить на светлую тему' : 'Переключить на темную тему'"
                    />
                </span>
            ...
            <template #item="{ item, props }">
                <Link :href="item.href" v-ripple class="flex items-center" v-bind="props.action">
                    <span :class="item.icon" />
                    <span>{{ item.label }}</span>
                    <Badge v-if="item.badge" class="ml-auto" :value="item.badge" />
                    <span v-if="item.shortcut" class="p-1 ml-auto text-xs border rounded border-surface bg-emphasis text-muted-color">{{ item.shortcut }}</span>
                </Link>
            </template>
            <template #end>
                <button v-ripple class="relative flex items-start w-full p-2 pl-4 overflow-hidden transition-colors duration-200 bg-transparent border-0 rounded-none cursor-pointer hover:bg-surface-100 dark:hover:bg-surface-800">
